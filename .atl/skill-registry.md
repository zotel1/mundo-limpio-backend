...
- Trigger: Trigger: mundo-limpio-backend, Java backend, Spring Boot backend, backend development, migration. Stack: Java 21, Spring Boot 3.3.0, PostgreSQL, Flyway, JWT, Clean Architecture. Workflow: SDD + Strict TDD + feature branches → stacked PRs to develop.
- Rules:
  - No compact rules declared; delegators should load the full skill file before direct work, or pass an explicit fallback path only when Project Standards cannot be injected.
...
- Trigger: Trigger: completing any significant task, fix, investigation, or SDD phase. Create a .md tutorial file explaining the reasoning, decisions, alternatives, and step-by-step process so the user can study and learn from the work.
- Rules:
  - No compact rules declared; delegators should load the full skill file before direct work, or pass an explicit fallback path only when Project Standards cannot be injected.
